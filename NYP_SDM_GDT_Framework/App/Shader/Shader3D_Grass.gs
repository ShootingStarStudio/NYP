...
void createQuad(vec3 base_position, mat4 crossmodel){
	...
	// wind
	//vec2 windDirection = vec2(1.0, 1.0); float windStrength = 0.02f;
	//vec2 uv = base_position.xz/10.0 + windDirection * windStrength * u_time ;
	vec2 uv = base_position.xz/10.0 + u_windDirection * u_windStrength * u_time ;
	...
	for(int i = 0; i < 4; i++) {
		...
		gs_out.colourVariation = fbm(gl_in[0].gl_Position.xz);
	    EmitVertex();
    }
    EndPrimitive();
}
...
void main()
{
	...
	grass_size = random(gl_in[0].gl_Position.xz) * u_grass_minsize + u_grass_minsize; 	// for random size
	...
	// create grass
	if (lessDetails != 1
		|| (lessDetails == 1 && (int(gl_in[0].gl_Position.x * LOD2) % 1) == 0 || (int(gl_in[0].gl_Position.z * LOD2) % 1) == 0)
		|| (lessDetails == 2 && (int(gl_in[0].gl_Position.x * LOD1) % 1) == 0 || (int(gl_in[0].gl_Position.z * LOD1) % 1) == 0)
	)

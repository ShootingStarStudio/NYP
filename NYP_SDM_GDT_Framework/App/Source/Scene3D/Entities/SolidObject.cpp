...
/**
 ...
 @param vec4BoxColour A glm::vec4 variable which is the colour of the collider box
 @param vec3BoxMin A glm::vec4 variable which is the minimum size of the collider box
 @param vec3BoxMax A glm::vec4 variable which is the maximum size of the collider box
 @return true is successfully initialised this class instance, else false
 */
bool CSolidObject::InitCollider(const std::string& _name,
								...
								glm::vec3 vec3BoxMax)
{
	return CCollider::InitCollider(_name, vec4BoxColour, vec3BoxMin, vec3BoxMax);
}

/**
@brief DeInitialise this instance
*/
void CSolidObject::DeInit(void)
{
}
...
void CSolidObject::Render(void)
{
	...
	else
	{
		...
		// Render the p3DMesh inside p3DMeshArray[]
		p3DMeshArray[eDetailLevel]->Render();
		glBindTexture(GL_TEXTURE_2D, 0);
	}

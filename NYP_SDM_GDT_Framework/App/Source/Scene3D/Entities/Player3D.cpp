...
CPlayer3D::~CPlayer3D(void)
{
	...
	if (pSecondaryWeapon)
	{
		delete pSecondaryWeapon;
		pSecondaryWeapon = NULL;
	}

	if (pPrimaryWeapon)
	{
		delete pPrimaryWeapon;
		pPrimaryWeapon = NULL;
	}
...
bool CPlayer3D::Init(void)
{
	...
	vec3Position.y = pTerrainManager->GetHeight(vec3Position.x, vec3Position.z) + fHeightOffset;
	...
	// Add a torchlight into Lightings
	iTorchLightID = pLightings->AddTorchLight(vec3Position, vec3Front, glm::vec3(0.0f), glm::vec3(1.0f), glm::vec3(1.0f),
 ...
 */
void CPlayer3D::AttachCamera(CCamera* pCamera)
{
...
bool CPlayer3D::Update(const double dElapsedTime)
{
	...
	if (cPhysics3D.GetStatus() == CPhysics3D::STATUS::IDLE)
	{
		if (bMoved == true)
		{
			...
			float fCheckHeight = pTerrainManager->GetHeight(vec3Position.x, vec3Position.z) + fHeightOffset;
	...
	// Update the torchlight position and direction
	pLightings->GetTorchLight(iTorchLightID)->SetPosition(vec3Position);
	pLightings->GetTorchLight(iTorchLightID)->SetFront(vec3Front);
...
void CPlayer3D::SnapToTerrain(void)
{
	...
	if (cPhysics3D.GetStatus() == CPhysics3D::STATUS::FALL)
	{
		...
		// if the player's y-coordinate is below the pTerrainManager 
		float fCheckHeight = pTerrainManager->GetHeight(vec3Position.x, vec3Position.z) + fHeightOffset;
...
void CPlayer3D::UpdateJumpFall(const double dElapsedTime)
{
	if (cPhysics3D.GetStatus() == CPhysics3D::STATUS::JUMP)
	{
		...
		vec3Position.y += float(cPhysics3D.GetFinalVelocity().y * dElapsedTime);
...
void CPlayer3D::UpdatePosture(const double dElapsedTime)
{
	...
	case PLAYERPOSTURE::CROUCH:
		...
		{
			fPostureOffset += 0.01f;
			if (fPostureOffset > 0.15f)
			{
				fPostureOffset = 0.15f;
				iPreviousPosture = iCurrentPosture;
				iCurrentPosture = iNextPosture;
			}
		}
	...
	case PLAYERPOSTURE::PRONE:
		fPostureOffset += 0.01f;
		if (fPostureOffset > 0.30f)
		{
			fPostureOffset = 0.30f;
			iPreviousPosture = iCurrentPosture;
			iCurrentPosture = iNextPosture;
		}
		break;

...
bool CTerrain::Update(const double dElapsedTime)
{
	...
	// This set of codes is to change the terrain by modifying the values in OpenGL memory directly
	// We don't need to load 3 separate CTerrains after this

	//for (int i = 0; i < iRows; i++)
	//{
	//	for (int j = 0; j < iCols; j++)
	//	{
	//		vVertexData[i][j].y += 0.001;
	//	}
	//}

	//// First, create a VBO with only vertex data
	//int counter = 0;
	//vboHeightmapData.ClearMemory(); // Clear memory
	//for (int i = 0; i < iRows; i++)
	//{
	//	for (int j = 0; j < iCols; j++)
	//	{
	//		//if (j/2 == (int)j/2)
	//		//	vVertexData[i][j].y += 0.1f;
...
void CTerrain::Render(void)
{
	...
	CShaderManager::GetInstance()->pActiveShader->setMat4("heightmapScale", glm::scale(glm::mat4(1.0), glm::vec3(vRenderScale)));
...
/**
 @brief Load 5 textures for the terrain
 */
bool CTerrain::LoadTextures(const string filenames[], const int iSize)
{
	if (iSize != NUMTEXTURES)
	{
		cerr << "The number of filenames for Terrain texture files is not equal to 5" << endl;
		return false;
	}

	for (int i = 0; i < iSize; i++)
	{
		if (tTextures[i].LoadTexture2D("Image/Terrain/textures/" + filenames[i], true) == false)
			return false;
...
float CTerrain::GetHeight(const float fX, const float fZ)
{
	...
	// If the iRow and iCol is out of the boundary of iRows and iCols, then return 0.0f

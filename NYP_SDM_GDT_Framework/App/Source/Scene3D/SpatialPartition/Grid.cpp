...
{
	return Init(glm::vec3(1.0f, 0.01f, 1.0f), glm::vec3(-1.0f, 0.00f, -1.0f));
}
...
/**
 ...
 @return true if successful, otherwise false
*/
bool CGrid::Init(glm::vec3 vec3TopRight, glm::vec3 vec3BottomLeft)
{
...
/**
 ...
 @return true if successful, otherwise false
*/
bool CGrid::Init(glm::vec3 vec3TopRight, glm::vec3 vec3BottomLeft, float fTopLeftHeight, float fBottomRightHeight)
{
	this->vec3TopRight = vec3TopRight;
	this->vec3BottomLeft = vec3BottomLeft;
...
bool CGrid::Update(const double dElapsedTime)
{
	...
	for (it = SolidObjectMap.begin(); it != end; ++it)
	{
		...
		for (it_other = it; it_other != end; ++it_other)
		{
			...
			//{
			lowerBoxMin = (it->second)->GetPosition() + (it->second)->vec3BoxMin;
			lowerBoxMax = (it->second)->GetPosition() + (it->second)->vec3BoxMax;
			upperBoxMin = (it_other->second)->GetPosition() + (it_other->second)->vec3BoxMin;
			upperBoxMax = (it_other->second)->GetPosition() + (it_other->second)->vec3BoxMax;
			//}
			//else
			//{
	...
	// Update fBlendValue
	fBlendValue += CFPSCounter::GetInstance()->GetDeltaTime();
	if (fBlendValue > 3.14159f)
		fBlendValue = 0.0f;
...
void CGrid::Render(void)
{
	if (bVisible)
	{
		...
		CShaderManager::GetInstance()->pActiveShader->setFloat("fBlendValue", sin(fBlendValue));

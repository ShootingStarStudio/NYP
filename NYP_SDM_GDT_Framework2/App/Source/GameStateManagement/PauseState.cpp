...
bool CPauseState::Init(void)
{
	...
	CShaderManager::GetInstance()->pActiveShader->setInt("texture1", 0);
...
bool CPauseState::Update(const double dElapsedTime)
{
	...
	// 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
	{
		...
		ImGui::SetWindowPos(ImVec2(CSettings::GetInstance()->cSimpleIniA.GetFloatValue("Size", "iWindowWidth", 800.0f) /2.0f - buttonWidth/2.0f,
			CSettings::GetInstance()->cSimpleIniA.GetFloatValue("Size", "iWindowHeight", 600.0f) /3.0f));				// Set the top-left of the window at (10,10)

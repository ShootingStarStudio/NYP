...
/**
 ...
 @param iMagnification - mag. filter, must be from ETextureFiltering enum
 @param iMinification - min. filter, must be from ETextureFiltering enum
 */
void CTexture::SetFiltering(int a_tfMagnification, int a_tfMinification)
{
	...
	iMinification = a_tfMinification;
	iMagnification = a_tfMagnification;
...
/**
 ...
 @param a_sPath - path to the texture
 @param bGenerateMipMaps - whether to create mipmaps
 @param bInvert - whether to invert the image
 */
bool CTexture::LoadTexture2D(string a_sPath, const bool bGenerateMipMaps, const bool bInvert)
{
	int width = 0;
	int height = 0;
	int nrChannels = 0;
	unsigned char* data = CImageLoader::GetInstance()->Load(a_sPath.c_str(), width, height, nrChannels, bInvert);

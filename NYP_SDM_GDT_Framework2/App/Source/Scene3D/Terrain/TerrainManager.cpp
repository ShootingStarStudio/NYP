						...
						const float fRenderX, const float fHeight, const float fRenderZ)
{
	...
	// Load 5 textures for the terrain
	string filenames[5] = { "fungus.png", "sand_grass_02.png", "rock_2_4w.png", "sand.png", "path.png" };
	if (cTerrain->LoadTextures(filenames, (int) sizeof(filenames) / sizeof(filenames[0])) == false)
	{
		cout << "Unable to load textures for terrain: " << sImagePath << endl;
		delete cTerrain;
		return false;
	}

	// Load greyscale image for the terrain
	if (cTerrain->LoadHeightMapFromImage(sImagePath) == false)
	{
		cout << "Unable to load height map from image: " << sImagePath << endl;
		delete cTerrain;
		return false;
	}

	// Set the size of the Terrain
	cTerrain->SetRenderSize(fRenderX, fHeight, fRenderZ);

...
bool CSolidObjectManager::CheckForCollision(void)
{
	...
	//		//{
	//			lowerBoxMin = (*it)->GetPosition() + (*it)->vec3BoxMin;
	//			lowerBoxMax = (*it)->GetPosition() + (*it)->vec3BoxMax;
	//			upperBoxMin = (*it_other)->GetPosition() + (*it_other)->vec3BoxMin;
	//			upperBoxMax = (*it_other)->GetPosition() + (*it_other)->vec3BoxMax;
	//		//}
	//		//else
	//		//{
	//		//	lowerBoxMin = (*it_other)->GetPosition() + (*it_other)->vec3BoxMin;
	//		//	lowerBoxMax = (*it_other)->GetPosition() + (*it_other)->vec3BoxMax;
	//		//	upperBoxMin = (*it)->GetPosition() + (*it)->vec3BoxMin;
	//		//	upperBoxMax = (*it)->GetPosition() + (*it)->vec3BoxMax;
	//		//}
	...
	for (it = lSolidObject.begin(); it != end; ++it)
	{
		...
		for (unsigned int i = 0; i < pProjectileManager->vProjectile.size(); i++)
		{
			// If the entity is not active, then skip it
			if ((pProjectileManager->vProjectile[i])->GetStatus() == false)
				continue;

			// Use ray tracing to check for collisions between the 2 entities
			if (CCollisionManager::RayBoxCollision((*it)->GetPosition() + (*it)->vec3BoxMin,
				(*it)->GetPosition() + (*it)->vec3BoxMax,
				(pProjectileManager->vProjectile[i])->GetPreviousPosition(),
				(pProjectileManager->vProjectile[i])->GetPosition()))
			{
				if ((*it)->GetType() == CSolidObject::TYPE::PLAYER)
				{
					// If this projectile is fired by the player, then skip it
					if ((pProjectileManager->vProjectile[i])->GetSource() == (*it))
						continue;
					(pProjectileManager->vProjectile[i])->SetStatus(false);
			...
			// Check for collisions between the 2 entities
			if (CCollisionManager::BoxBoxCollision((*it)->GetPosition() + (*it)->vec3BoxMin,
				(*it)->GetPosition() + (*it)->vec3BoxMax,
				(pProjectileManager->vProjectile[i])->GetPosition() + (pProjectileManager->vProjectile[i])->vec3BoxMin,
				(pProjectileManager->vProjectile[i])->GetPosition() + (pProjectileManager->vProjectile[i])->vec3BoxMax) == true)
			{
				if ((*it)->GetType() == CSolidObject::TYPE::PLAYER)
				{
					(pProjectileManager->vProjectile[i])->SetStatus(false);

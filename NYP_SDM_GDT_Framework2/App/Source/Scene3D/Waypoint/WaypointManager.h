...
public:
	...
	// Add the position of a waypoint by searching for its iID
	int AddWaypoint(const int iID, const glm::vec3 position);
	// Remove waypoint based on its iID
	bool RemoveWaypoint(const int iID);
	...
	CWaypoint* GetWaypoint(const int iID);
...
protected:
	...
	float fDistanceTolerance;

	vector<CWaypoint*> vWaypoints;

	// Remove related waypoint
	void RemoveRelatedWaypoint(const int iID);
};
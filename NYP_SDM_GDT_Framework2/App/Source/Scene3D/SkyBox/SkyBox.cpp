 ...
 */
bool CSkyBox::SetIndex(const int iIndex)
{
	if ((iIndex >= 0) && (iIndex < int(skyboxTextureIDs.size())))
	{
		this->iIndex = iIndex;
		return true;;
	}
...
void CSkyBox::Render(void)
{
	...
	glBindTexture(GL_TEXTURE_CUBE_MAP, skyboxTextureIDs[iIndex]);
		glBindVertexArray(VAO);

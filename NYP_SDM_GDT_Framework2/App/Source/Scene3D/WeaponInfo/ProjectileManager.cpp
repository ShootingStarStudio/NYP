...
/**
 ...
 @return A bool variable to indicate this method successfully completed its tasks
 */
bool CProjectileManager::Update(const double dElapsedTime)
{
	...
	if (uiIndexFirst >= vProjectile.size())
	{
		...
		for (unsigned int i = uiIndexFirst; i <= uiIndexLast; i++)
		{
			...
			// Check if a projectile went below the terrain
			fTerrainHeight = pTerrainManager->GetHeight(vProjectile[i]->GetPosition().x, vProjectile[i]->GetPosition().z);
			if (fTerrainHeight - vProjectile[i]->GetPosition().y > 0.0f)
			{

						...
						const float fSpeed)
{
	...
	// Initialise the projectile line
	vec3StartPosition = vec3Position;
	InitProjectileLines("Shader3D_Line", glm::vec4(1.0f, 1.0f, 1.0f, 1.0f), dLifetime);
...
bool CProjectile::Update(const double dElapsedTime)
{
	...
	if (CProjectileLines::bIsDisplayed)
	{
		vec3EndPosition = vec3Position + vec3Front;
		CProjectileLines::Update(dElapsedTime);
	}
...
void CProjectile::Render(void)
{
	...
	if (CProjectileLines::bIsDisplayed)
	{
		this->projectileLineView = view;
		this->projectileLineProjection = projection;

		CProjectileLines::PreRender();
		CProjectileLines::Render();
		CProjectileLines::PostRender();
	}
}

									...
									CSolidObject* pSource)
{
	...
	vProjectile[uiIndexLast]->Init(vec3Position, vec3Front, dLifetime, fSpeed, iDamage);
	vProjectile[uiIndexLast]->SetPreviousPosition(vec3Position);
	vProjectile[uiIndexLast]->InitCollider(	"Shader3D_Line", 
		glm::vec4(1.0f, 1.0f, 0.0f, 1.0f),
		glm::vec3(-vProjectile[uiIndexLast]->getScale() * 0.5f, -vProjectile[uiIndexLast]->getScale() * 0.5f, -vProjectile[uiIndexLast]->getScale() * 0.5f),
		glm::vec3(vProjectile[uiIndexLast]->getScale() * 0.5f, vProjectile[uiIndexLast]->getScale() * 0.5f, vProjectile[uiIndexLast]->getScale() * 0.5f));

						...
						const int iDamage)
{
	...
	// Generate the mesh
	mesh = CMeshBuilder::GenerateBox(glm::vec4(1.0f, 0.5f, 0.0f, 1.0f), fScale, fScale, fScale);
...
{
	if (bStatus == false || dLifetime < 0.0f)
		return;
...
int CProjectile::GetDamage(void) {
	return iDamage;
}

float CProjectile::getScale(void) {
	return fScale;
}

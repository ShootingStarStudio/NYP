...
CCamera::CCamera(void)
	: vec3Position(glm::vec3(0.0f, 0.0f, 0.0f)), vec3PositionOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Front(glm::vec3(0.0f, 0.0f, -1.0f)), vec3FrontOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Up(glm::vec3(0.0f, 1.0f, 0.0f)), vec3UpOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Right(glm::vec3(1.0f, 0.0f, 0.0f)), vec3RightOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3WorldUp(glm::vec3(0.0f, 1.0f, 0.0f))
	, fYaw(YAW), fYawOffset(0)
	, fPitch(PITCH), fPitchOffset(0)
	, fRoll(ROLL), fRollOffset(0)
...
CCamera::CCamera(	const glm::vec3 position,
					...
					const float yaw,
					const float pitch)
	: vec3Position(position), vec3PositionOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Front(glm::vec3(0.0f, 0.0f, -1.0f)), vec3FrontOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Up(glm::vec3(0.0f, 1.0f, 0.0f)), vec3UpOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Right(glm::vec3(1.0f, 0.0f, 0.0f)), vec3RightOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3WorldUp(up)
...
CCamera::CCamera(	const float posX, const float posY, const float posZ,
	...
	: vec3Position(glm::vec3(posX, posY, posZ)), vec3PositionOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Front(glm::vec3(0.0f, 0.0f, -1.0f)), vec3FrontOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Up(glm::vec3(0.0f, 1.0f, 0.0f)), vec3UpOffset(glm::vec3(0.0f, 0.0f, 0.0f))
	, vec3Right(glm::vec3(1.0f, 0.0f, 0.0f)), vec3RightOffset(glm::vec3(0.0f, 0.0f, 0.0f))
...
void CCamera::UpdateCameraVectors(void)
{
	...
	rotationMat = glm::rotate(rotationMat, glm::radians(fRoll + fRollOffset), glm::vec3(0.0f, 0.0f, 1.0f));
	vec3Right = glm::normalize(glm::cross(vec3Front, vec3WorldUp));
	vec3RightOffset = glm::vec3(rotationMat * glm::vec4(vec3Right, 1.0f));
	vec3Up = glm::normalize(glm::cross(vec3RightOffset, vec3Front));

	if (fRoll + fRollOffset > 0.0f)
	{
		vec3Position -= vec3Right;
	}
	else if (fRoll + fRollOffset < 0.0f)
	{
		vec3Position += vec3Right;
	}

	// Update the camera's position with the offsets due to camera shakes
	vec3Position += vec3PositionOffset;
}

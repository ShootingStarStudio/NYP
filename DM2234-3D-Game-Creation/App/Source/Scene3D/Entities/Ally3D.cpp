	...
	, cTerrain(NULL)
{
	...
	Faction = 1;
	// Update the vectors for this NPC
 ...
 */
CAlly3D::CAlly3D(	const glm::vec3 vec3Position,
...
bool CAlly3D::Init(void)
{
	...
	iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/Scene3D_Ally_01.tga", false);
	if (iTextureID == 0)
	{
		cout << "Unable to load Image/Scene3D_Ally_01.tga" << endl;
		return false;
	}
	...
	float fCheckHeight = cTerrain->GetHeight(vec3Position.x, vec3Position.y);
	int m_iWayPointID = cWaypointManager->AddWaypoint(glm::vec3(vec3Position.x, fCheckHeight, vec3Position.y));
 ...
 */
void CAlly3D::SetModel(const glm::mat4 model)
{
	this->model = model;
}
 ...
 */
void CAlly3D::SetView(const glm::mat4 view)
{
	this->view = view;
}
 ...
 */
void CAlly3D::SetProjection(const glm::mat4 projection)
{
 ...
 */
void CAlly3D::AttachCamera(CCamera* cCamera)
{
 ...
 */
bool CAlly3D::IsCameraAttached(void)
{
	if (cCamera)
		return true;
	return false;
 ...
 */
void CAlly3D::SetWeapon(const int iSlot, CWeaponInfo* cWeaponInfo)
{
	if (iSlot == 0)
 ...
 */
CWeaponInfo* CAlly3D::GetWeapon(void) const
{
	if (iCurrentWeapon == 0)
...
/**
 ...
 @param iSlot A const int variable which contains the weapon info to check for. 0 == Primary, 1 == Secondary
 */
void CAlly3D::SetCurrentWeapon(const int iSlot)
{
...
/**
 ...
 @return A bool value
 */
bool CAlly3D::DischargeWeapon(void) const
{
 ...
 */
void CAlly3D::ProcessMovement(const ENEMYMOVEMENT direction, const float deltaTime)
{
 ...
 */
void CAlly3D::ProcessRotate(const float fXOffset)
{
	// Update the yaw
 ...
 */
bool CAlly3D::Update(const double dElapsedTime)
{
	...
	case FSM::PATROL:
		...
		if (iFSMCounter > iMaxFSMCounter)
		{
			...
			if (glm::distance(vec3Position, TargetEntity->GetPosition()) < fDetectionDistance) {
	...
	case FSM::ATTACK:
		if (TargetEntity != nullptr) {
			if (glm::distance(vec3Position, TargetEntity->GetPosition()) < fDetectionDistance)
			{
				vec3Front = glm::normalize((TargetEntity->GetPosition() - vec3Position));
				UpdateFrontAndYaw();
 ...
 */
void CAlly3D::UpdateAllyVectors(void)
{
 ...
 */
void CAlly3D::UpdateFrontAndYaw(void)
{
...
float CAlly3D::getDetectionDistance(void) {

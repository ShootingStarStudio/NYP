...
bool CTreeKabak3D::Init(void)
{
	...
	if (bInstancedRendering)
	{
		...
		for (unsigned int i = 0; i < iNumOfInstance; i++)
		{
			...
			float x, y, z;
			do {
				x = vec3Position.x + (rand() % 100) * fSpreadDistance * 0.01f - fSpreadDistance * 0.5f;
				z = vec3Position.z + (rand() % 100) * fSpreadDistance * 0.01f - fSpreadDistance * 0.5f;
				y = CTerrain::GetInstance()->GetHeight(x, z);
			} while (y < 12.5 || y > 37.5);
			model = glm::translate(model, glm::vec3(x, y, z));

			// 2. Use scaling to create trees of various sizes
			float scale = (100 + ((rand() % 100) - 50)) / 100 * 0.05f;

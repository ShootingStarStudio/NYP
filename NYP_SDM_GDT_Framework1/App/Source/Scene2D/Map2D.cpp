...
void CMap2D::Render(void)
{
	...
	for (unsigned int uiRow = 0; uiRow < pSettings->cSimpleIniA.GetIntValue("NumTiles", "NUM_TILES_YAXIS", 24); uiRow++)
	{
		for (unsigned int uiCol = 0; uiCol < pSettings->cSimpleIniA.GetIntValue("NumTiles", "NUM_TILES_XAXIS", 32); uiCol++)
		{
			model = glm::mat4(1.0f);
...
void CMap2D::PrintSelf(void) const
{
	...
	for (unsigned uiLevel = 0; uiLevel < uiNumLevels; uiLevel++)
	{
		...
		for (unsigned uiRow = 0; uiRow < pSettings->cSimpleIniA.GetIntValue("NumTiles", "NUM_TILES_YAXIS", 24); uiRow++)
		{
			for (unsigned uiCol = 0; uiCol < pSettings->cSimpleIniA.GetIntValue("NumTiles", "NUM_TILES_XAXIS", 32); uiCol++)
			{
				cout.fill('0');
				cout.width(3);
...
int CMap2D::ConvertTo1D(const glm::vec2& pos) const
{
	...
	return (pos.y * pSettings->cSimpleIniA.GetIntValue("NumTiles", "NUM_TILES_XAXIS", 32)) + pos.x;
}
...
bool CMap2D::ResetAStarLists(void)
{
	...
	while (pqOpenList.size() != 0)
		pqOpenList.pop();
	// Reset vCameFromList
	for (int i = 0; i < vCameFromList.size(); i++)
	{

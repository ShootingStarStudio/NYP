...
)
{
	...
	while (!fileStream.eof()) {
		...
		if (strncmp("v ", buf, 2) == 0) { //process vertex position
			...
			TexCoord tc;
			sscanf_s((buf + 2), "%f%f%f", &tc.u, &tc.v);
			...
			else
			{
				...
				std::cout << "File can't be read by parser." << std::endl;
				return false;
			}
		}
	}
...
bool LoadOBJMTL(const char* file_path, const char* mtl_path, 
	std::vector<Position>& out_vertices, std::vector<TexCoord>& out_uvs, 
	std::vector<Vector3>& out_normals, std::vector<Material>& out_materials)
{
	std::ifstream fileStream(file_path, std::ios::binary);
	if (!fileStream.is_open())
	{

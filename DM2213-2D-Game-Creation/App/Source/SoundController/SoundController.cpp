									...
									vec3df vec3dfSoundPos)
{
	...
	if (eSoundType == CSoundInfo::SOUNDTYPE::_2D)
		cSoundInfo->Init(ID, pSoundSource, bIsLooped, soundEffect);
	else
		cSoundInfo->Init(ID, pSoundSource, bIsLooped, soundEffect, eSoundType, vec3dfSoundPos);
...
void CSoundController::EffectDistort(const int ID) {

	CSoundInfo* pSoundInfo = GetSound(ID);
	if (pSoundInfo && cSoundEngine->isCurrentlyPlaying(pSoundInfo->GetSound()) && pSoundInfo->GetSFXStatus())
	{
		cout << "Sound #" << ID << " is currently being played." << endl;
		cout << "Sound #" << ID << " is sound effect positive." << endl;
		//sfx function here
		//ambiera.com/irrklang/tutorial-sound-effects.html
		return;
	}

}
void CSoundController::EffectEcho(const int ID) {

}
void CSoundController::EffectReverb(const int ID) {

}
void CSoundController::EffectClear(const int ID) {
...
{
	CSoundInfo* pSoundInfo = GetSound(ID);
	if (pSoundInfo && cSoundEngine->isCurrentlyPlaying(pSoundInfo->GetSound()))
	{
		cout << "Sound #" << ID << " is currently being played." << endl;
		//stop function here
